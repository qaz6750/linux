...
static u8 fts_event_handler_type_b(struct stmfts_data *sdata)
{
	...
	do {
		printk("[FTS-Touch] [%d] %02X %02X %02X %02X %02X %02X %02X %02X\n",
		...
		case FTS_COORDINATE_EVENT:
			...
			if ((sdata->finger[TouchID].ttype == FTS_EVENT_TOUCHTYPE_NORMAL) ||
				...
				if (sdata->finger[TouchID].action == FTS_COORDINATE_ACTION_RELEASE) {
                    ...
                    printk("[FTS-Touch] [R] tID:%d loc:%s dd:%d,%d mc:%d tc:%d lx:%d ly:%d mx:%d my:%d p:%d nlvl:%d maxS:%d hid:%d\n",
							TouchID, location,
				...
				printk("[FTS-Touch] %s: do not support coordinate type(%d)\n",
						__func__, sdata->finger[TouchID].ttype);
			}
...
static int stmfts_power_on(struct stmfts_data *sdata)
{
	...
	printk("[FTS-Touch] sdata->fw_ver: %X", sdata->fw_ver);
...
//Downstream device tree uses 4095 as max_coords
	...
	printk("[FTS-Touch]initializing ST-Microelectronics FTS...");
...
static int __maybe_unused stmfts_runtime_suspend(struct device *dev)
{
	...
	if (ret)
		printk("failed to suspend device: %d\n", ret);
...
static int __maybe_unused stmfts_runtime_resume(struct device *dev)
{
	...
	if (ret)
		printk("failed to resume device: %d\n", ret);
...
static const struct of_device_id stmfts_of_match[] = {
	{ .compatible = "st,stmfts_fts1ba90a", },
	{ },
};
